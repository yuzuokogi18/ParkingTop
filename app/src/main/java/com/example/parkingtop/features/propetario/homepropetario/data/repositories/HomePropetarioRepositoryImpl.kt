...
import com.example.parkingtop.features.propetario.homepropetario.domain.entities.GeneralSummary
        ...
        return try {
            ...
            if (homeResponse.isSuccessful) {
                ...
                val finalData = HomePropetarioData(
                    ...
                    parkings = if (realParkings.isNotEmpty()) realParkings else homeDto?.parkings?.map { it.toDomain() } ?: emptyList()
                )
                Result.success(finalData)
            } else {
                if (realParkings.isNotEmpty()) {
                    Result.success(HomePropetarioData(
